...
enc_response({error, namespace_not_found}) ->
  aspike_protocol:enc_error_response(?AEROSPIKE_ERR_NAMESPACE_NOT_FOUND);

enc_response({error, record_not_found}) ->
  aspike_protocol:enc_error_response(?AEROSPIKE_ERR_RECORD_NOT_FOUND);
